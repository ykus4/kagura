...
/// Returns true if the name looks like it was generated by kagura.
static bool isKaguraGenerated(StringRef Name) {
  return Name.starts_with("kagura_") || Name.starts_with("__kagura");
}
...
PreservedAnalyses SymbolMapPass::run(Module &M, ModuleAnalysisManager &) {
  ...
  for (const Function &F : M) {
    ...
    bool NameChanged = isKaguraGenerated(Current) || (Original != Current);
    if (!NameChanged)
      continue;
  ...
  for (const GlobalVariable &GV : M.globals()) {
    if (!isKaguraGenerated(GV.getName()))
      continue;
